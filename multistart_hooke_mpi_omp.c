...
int main(int argc, char *argv[])
{
	...
	//antiproswpevei to  rank tis diergasias
	int my_rank;
	
	//antiproswpevei to plithos twn diergasiwn 
	int p;
	
	//plithos twn epanalipsewn me tis opoies tha asxolithei i kathe diergasia
	int num;
	
	//dilosi metablitis start (apo poia epanalipsi tha ksekinisei i diergasia)
	
	int start;
	
	//dilosi metablitis end( se poia epanalipsi tha teleiosei i diergasia)
	
	int end;
	
	//etiketes twn minimatwn
    ...
    for (i = 0; i < MAXVARS; i++) local_best_pt[i] = 0.0;
	...
	//parallilos ypologismos 
	MPI_Init(&argc, &argv);
	
	//i kathe mia diergasia pairnei to rank tis 
	MPI_Comm_rank(MPI_COMM_WORLD,&my_rank);
	
	
	//i kathe mia diergasia enimeronetai gia to plithos twn diergasiwn pou simmetexoun stin epikoinwnia 
	
	MPI_Comm_size(MPI_COMM_WORLD,&p);
	
	//vrisko me poses epanalipseis tha asxolithei i kathe diergasia
	
	 num= ntrials/p;
	 
	//ypologismos tou start
	
	start = my_rank * num;
	
	//ypologismos tou end
	
	//an den eisai i teleytaia diergasia 
	if(my_rank!=p-1)
	...
	//an eisai i teleytaia diergasia 
	else
	...
	//pleon i epanalipsi leitoyrgei metaksy start kai end 
	...
	//an eisai  diergasia me rank diaforo tou midenos 
	if(my_rank!=0)
	
	{
	
	//stelnw to local_best_trial
	 MPI_Send(&local_best_trial, 1, MPI_INT, 0, tag1, MPI_COMM_WORLD);
	 
	 //stelnw to local_best_jj
	 MPI_Send(&local_best_jj, 1, MPI_INT, 0, tag2, MPI_COMM_WORLD);
	 
	 //stelnw to local_best_fx
	 MPI_Send(&local_best_fx,1, MPI_DOUBLE, 0, tag3, MPI_COMM_WORLD);
	
	 //stelnw to local_best_pt
	 MPI_Send(&local_best_pt[0],nvars, MPI_DOUBLE,0, tag4, MPI_COMM_WORLD);
	
	
	}
	
	//i diergasia me rank 0 tha paralavei, kai tha kanei tis aparaitites sigkriseis, etsi wste
	//na vrethei to oliko veltisto 
	else
	
	
	{
	
	 //arxikopoiei ta best me ayta pou exei ypologisei i diergasia 0 (etsi wste meta na mporei na kanei sygkriseis
	  ...
	  //edo ginontai ta receive kai oi sigkriseis 
	  for(source=1;source<p;source++)
	  
	  {
	     MPI_Recv(&local_best_trial, 1, MPI_INT, source, tag1, MPI_COMM_WORLD,  &status);
		 
		 MPI_Recv(&local_best_jj, 1, MPI_INT, source, tag2, MPI_COMM_WORLD,  &status);
		 
	     MPI_Recv(&local_best_fx, 1, MPI_DOUBLE, source, tag3, MPI_COMM_WORLD, &status);
		 
		 MPI_Recv(&local_best_pt, nvars, MPI_DOUBLE, source, tag4, MPI_COMM_WORLD,  &status);
		 
		 
		 //edo ginontai oi katalliles sigkriseis 
